...
// global varaibles
int** grid = nullptr;		// 2D array
int sizeOfGrid;		// number of rows and columns in the grid
int pathLength;	// length of shortest wire path
arrayQueue<position> q;
				// queue of unexpanded reched positions

position start,	// one end point of wire
		 finish;// other end point
position* path = nullptr;	// the shortest path
...
{// Input the wire routing data.
	...
	make2dArray(grid, sizeOfGrid + 2, sizeOfGrid + 2);
...
{// Find a shortest path from start to finish.
 // Return true if successful, false if impossible.
	...
	{
		grid[0][i] = grid[sizeOfGrid + 1][i] = 1;	// bottom and top
		grid[i][0] = grid[i][sizeOfGrid + 1] = 1;	// left and right
	}

	position here = start;
	grid[start.row][start.col] = 2;		// block
	int numberOfNbrs = 4;	// neighbors of a grid position
...
{
	if (grid != nullptr)
		delete2dArray(grid, sizeOfGrid + 2);
	if (path != nullptr)
		delete[] path;
}
...
int testWireRouter()
{
	...
	if (findPath())
		...
		cout << "Three is no wire path" << endl;
