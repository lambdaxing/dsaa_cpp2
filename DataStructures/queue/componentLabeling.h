...
// global variables
int** pixel;
int sizeOfP;		// number of rows and columns in the image
...
{// Label the components.
	...
	{
		pixel[0][i] = pixel[sizeOfP + 1][i] = 0; // bottom and top
		pixel[i][0] = pixel[i][sizeOfP + 1] = 0; // left and right
	}
	...
	for (int r = 1; r <= sizeOfP; r++)		// row r of image
		for (int c = 1; c <= sizeOfP; c++)	// column c of image
			if (pixel[r][c] == 1)
			{// new component
...
{
	if (pixel != nullptr)
		delete2dArray(pixel, sizeOfP + 2);

}

void testComponentLabeling()
{
	welcome();
	inputImage();
	labelComponents();
