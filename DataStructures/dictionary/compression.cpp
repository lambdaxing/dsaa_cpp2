...
typedef std::pair<const long, int> pairType;		// the key: 20(12+8) bits; the value: 12 bits;
...
{// Create input and output streams.
	...
	std::strcpy(outputFile, inputFile);
...
{// Output 8 bits , save rest in leftOver.
	int c, d;
	if (bitsLeftOver)
	{// half byte remains form before
		d = static_cast<int>(pcode & MASK1);	// right ByteSize bits		// 后面的后八位
		c = static_cast<int>((leftOver << EXCESS) | (pcode >> BYTE_SIZE));	// 之前剩下的四位，加上后面的前八位
		out.put(c);
		out.put(d);
		bitsLeftOver = false;
	}
	...
	{// no bits remain from before
		leftOver = pcode & MASK2;		// right EXCESS bits	// 保留后四位
		c = static_cast<int>(pcode >> EXCESS);					// 去除后四位
		out.put(c);
		bitsLeftOver = true;
	}
}
