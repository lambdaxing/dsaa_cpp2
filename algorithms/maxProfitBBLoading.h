...
struct heapNode
{
	...
	{
		return upperWeight < rhs.upperWeight;
	}
 ...
 // Return weight of best loading.
	...
	{// not at a leaf
		...
		if (eNodeWeight + weight[eNodeLevel] <= capacity)
			// feasible left child
			addLiveNode(eNodeWeight + weight[eNodeLevel] + remainingWeight[eNodeLevel], eNodeLevel + 1, eNode, true);
		// right child is always feasible
		addLiveNode(eNodeWeight + remainingWeight[eNodeLevel], eNodeLevel + 1, eNode, false);
		...
		eNodeWeight = nextENode.upperWeight - remainingWeight[eNodeLevel - 1];
	}
...
void addLiveNode(int upperWeight, int level, bbNode* theParent, bool leftChild)
{// Add a new live node to the live node max heap.
 ...
 // leftChild is true if the new live node is the left child of theParent.
