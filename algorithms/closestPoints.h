...
struct point
{
	...
	// constructor
	point(double theX = 0, double theY = 0) :x(theX), y(theY) {}
};
...
{// Point with id, comparaisons done using x-coordinates.
	...
	// constructor
	point1(double theX = 0, double theY = 0, int theID = 0) :point(theX, theY), id(theID) {}
...
{// Point with an integer field, comparisons done using y-coordinates.
	...
	// constructor
	point2(double theX = 0, double theY = 0, int theP = 0) :point(theX, theY), p(theP) {}
...
{// Pairs of points and their distance.
	...
	// constructor
	pointPair(point1& theA, const point1& theB, double theDist) :a(theA), b(theB), dist(theDist) {}
...
{// x[l:r] points sorted by x-coordinate, r > l.
 ...
 // Return closest pair of points in x[l:r].
	...
	for (int i = l; i <= r; i++)
		if (std::fabs(x[m].x - y[i].x) < best.dist)
			z[k++] = y[i];
...
void testClosestPoints()
{
	...
	std::cout << "Closest points are " << best.a.id << " and " << best.b.id << std::endl;
